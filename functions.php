...
//Добавлена Функция такая же как и FormatAddress, только с указанием Сельсвета (для полного отображения адреса на странице инфо БС)
function FormatAddressSelsovet($set_type, $settlement, $street_type, $street_name, $house_type, $house_number,$selsovet, $area, $region) { // форматируем данные к виду адреса + сельсовет
    if (strlen($settlement) > 0) {
        if ($set_type == 'город') {
            $set_type = "г.";
        }
	...
	if (strlen($selsovet) > 0) {
        $address = $address . ", " . $selsovet;
    }
    if (strlen($settlement) > 0) {
        if (!empty($area))
            $address = $address . ", " . $area . " р-н";
...
function FormatNasPunkt($set_type, $settlement) {  //Формирование типа населенного пункта (Галенчик А.В. для формы аренды)
	 if (strlen($settlement) > 0) {
        if ($set_type == 'город') {
            $set_type = "г.";
        }
...
function MySQLAction($data, $edit_table, $id, $edit_type, $history, $master_id = 0) {//функция изменения данных на mysql + запись в историю
    ...
    if ($edit_type == 'insert') {          // INSERT данных
        $sql = "INSERT INTO $edit_table SET ";
    }
    if ($edit_type == 'update') {          // UPDATE данных
    ...
    if ($edit_type != 'delete') {
        ...
        $i = 0;
        foreach ($data as $key => $value) { //Продолжение запроса на проверку изменений
            if ($i > 0)
                $sql.=',';
            ...
            if ($row[$key] != $value) {
                if (empty($changes)) //Если переменная изменений не пустая и существет
                    $changes = $prechanges; //Присвоить значение $prechanges
                if ($edit_type == 'update')
                    $changes.="$key " . '"' . ValueById($row[$key], $key) . '"' . " заменено на " . '"' . ValueById($value, $key) . '"' . "; ";
                if ($edit_type == 'insert')
                    $changes.="$key получило значение " . '"' . ValueById($value, $key) . '"' . "; "; //Формирование записи в таблицу истории (ниже)
            }
        }
        $sql.=" $where"; //Для объекта под выбранным Id
    }

    if ($edit_type == 'delete') {           // DELETE
        $sql = "DELETE FROM $edit_table WHERE id=$id";
        foreach ($row as $key => $value) {
            if (!empty($value)) //Если значение не пустое
                $changes.="$key было значение " . '"' . ValueById($value, $key) . '"' . "; ";
        }
    }
    $query = mysql_query($sql) or die(mysql_error());
    if ($id == 0 || empty($id)) //Если Id = 0 ИЛИ пустое значение id, то 
        $id = mysql_insert_id(); //Добавить новый Id

    // HISTORY
    if ($history) {
        if ($edit_type == 'insert') { //Добавление данных в таблицу
            //Не было действий в условиях, так как выше они прописаны
        }

        if ($edit_type == 'delete') { //Удаление данных из таблицы
            //Не было действий в условиях, так как выше они прописаны
        }

        if ($edit_type == 'update') { //Обновление данных в таблице
            //Не было действий в условиях, так как выше они прописаны
        }

        if (!empty($changes)) {  //Внесение в таблицу истории полученные изменения из переменной $changes
            $sql = "INSERT INTO history SET"; 
            $sql.=" act_date=SYSDATE()"; //Системная дата на сервере
            $sql.=",act_time=CURTIME()"; //Текущее время на сервере
            $sql.=",user_id=" . $_SESSION['user_id']; //id пользователя, который внес изменение
            $sql.=",table_name='$edit_table'"; //Таблица для внесения изменений
            if ($master_id == 0) {
                $sql.=",object_id=$id";
            } else {
                $sql.=",object_id=$master_id";
            }
            $sql.=",action='$edit_type'"; //Тип изменения (Insert, Update, Delete)
            $sql.=",changes='$changes'"; //Запись об изменениях (смотри выше в первой части Функции как формируется переменная $changes
            $query = mysql_query($sql) or die(mysql_error());
        }
    }
...
function MySQLActionRENT($data,$edit_table,$id,$edit_type,$history,$names,$clone) {//функция изменения данных на mysql + запись в историю RENT
	$sql2 = "SELECT * FROM $edit_table WHERE id=" . NumOrNull($id);     // запрос для сравнения изменений!!!
    $query = mysql_query($sql2) or die(mysql_error());
    $row = mysql_fetch_assoc($query);
	
	$changes = "<b><u>".$clone."</u></b><br/>";
    ...
    if ($edit_type != 'delete') {
       ...
       //     $prechanges = "ррл " . ValueById($row['bts_id_point1'], 'bts_id_point1') . " - " . ValueById($row['bts_id_point2'], 'bts_id_point2') . ";";
        ...
        foreach ($data as $key => $value) { //Продолжение запроса на проверку изменений
            ...
            if ($row[$key] != $value) { //Сравнение данных таблицы rent с массивом $data по одинаковому полю
               // if (empty($changes)) //Если переменная изменений не пустая и существет
               //     $changes = $prechanges; //Присвоить значение $prechanges
                if ($edit_type == 'update')
                    $changes.="".$names[$key]." = " . '"' . $row[$key] . '"' . " --> " . '"' . $value . '"' . "; ";
				if ($edit_type == 'insert')
                    $changes.="".$names[$key]." = " . '"' . $value . '"' . "; "; //Формирование записи в таблицу истории (ниже)
            }
        }
        $sql.=" $where"; //Для объекта под выбранным Id
    }
    ...
    if ($edit_type == 'delete') {           // DELETE
        ...
        foreach ($row as $key => $value) {
            if (!empty($value)) //Если значение не пустое
                $changes.="$key было значение " . '"' . $value . '"' . "; ";
        }
    }
    $query = mysql_query($sql) or die(mysql_error());
    if ($id == 0 || empty($id)) //Если Id = 0 ИЛИ пустое значение id, то 
