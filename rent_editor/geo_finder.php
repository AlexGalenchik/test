...
<!--                                <a href="reset.php" ><button type="button" class="btn btn-danger" >СБРОСИТЬ ФИЛЬТРЫ</button></a>-->
   ...
   <table>
     <tr>  
		...
		<th class='typePlace'>Тип объекта<br>РПС</th>
		...
		If ($_SESSION['reg_user'] !== 'УРС') {
		 echo "<th class='typeDogovor'>Тип <br> Аренды</th>";
		}
		echo 
		"<th class='dataDogovor'>Дата <br> договора</th>
		<th class='dateStart'>Начало<br/>договора</th>
		<th  class='dateEnd'>Конец<br/>договора</th>
		<th  class='dateEnd'>Пролонгация<br/>договора</th>";
		If ($_SESSION['reg_user'] !== 'УРС') {	
		echo "<th class='typeArendy'>Тип <br> Договора</th>";
		}
		echo
        ...
        // проверка наличия номера NE
            ...
            echo "<td><center><b><a href='index.php?Id={$data[$i]['Id']}'>{$number_NE}</a></b></center></td>
                  ...
                  echo "<td><center>".$data[$i]['dogovor_date']."</center></td>
                  <td><center>".$data[$i]['start_date_dogovor']."</center></td>
                  <td><center>".$data[$i]['finish_date_dogovor']."</center></td>
				  <td><center>".$data[$i]['prolongaciya']."</center></td>";
					If ($_SESSION['reg_user'] !== 'УРС') {				  
                  echo "<td><center>".$data[$i]['type_arenda']."</center></td>";
					}
				  echo "<td><center>".$data[$i]['summa']." <br> ".$data[$i]['type_currency']."</center></td>
                  <td><center>".$data[$i]['nds2']." <br> ".$data[$i]['type_currency']."</center></td>";
				  If ($_SESSION['reg_user'] !== 'УРС') {
                  echo "<td><center>".$data[$i]['contragent_data']."</center></td>";
				  }
