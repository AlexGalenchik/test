...
if ($edit_type=='insert') {
  if (empty($bts_id) && !isset($_GET['rep']) && !isset($_GET['rfud'])) {
    ...
    $bts_id = MySQLAction($data2,'bts',$id,'insert',true);  //Функция вносит изменения и записывает в историю
  
    $data3['bts_id'] = $bts_id;
    MySQLAction($data3,'budget',$_POST['budget_id'],'update',false); //Функция вносит изменения и записывает в историю
  }
  $data['bts_id'] = $bts_id;
}

$id = MySQLAction($data,'formulars',$id,$edit_type,true); //Функция вносит изменения и записывает в историю
...
if (isset($_GET['fud']) || isset($_GET['rfud'])) {
  ...
  $id = MySQLAction($data,'formular_actions',0,'insert',false); //Функция вносит изменения и записывает в историю
