...
if ($_GET['f'] == '5') {  // редактор списка значений
	...
	if ($_GET['obj'] == 'construction_5g_types') {					// 5G Добавлен
        $tx = "f=5&ff=" . $_GET['ff'] . "&obj=construction_5g_types";
    }
	...
	if ($_GET['obj'] == 'antenna_types_5g') {						// 5G Добавлен
        $tx = "f=5&ff=" . $_GET['ff'] . "&obj=antenna_types_5g";
    }
	...
	if ($_GET['obj'] == 'rep_diapazons') {
        $tx = "f=5&ff=" . $_GET['ff'] . "&obj=rep_diapazons";
    }
...
if ($_GET['f'] == '40') { // инфо Документация
    if (isset($_POST['bts_number'])) {
        $bts_number = $_POST['bts_number'];
        $sql = "SELECT Id FROM bts WHERE bts_number=" . StrOrNull($bts_number);
        $query = mysql_query($sql) or die(mysql_error());
...
if ($_GET['f'] == '36') { // редактор включений
	...
	if ($_GET['f'] == '38') { // окно карты
    $tx = "index.php?f=38";
	}
if ($_GET['f'] == '39') { // 	Поиск объекта и вывод документов
	 if (isset($_POST['bts_number'])) {
        $bts_number = $_POST['bts_number'];
        $sql = "SELECT Id FROM bts WHERE bts_number=" . StrOrNull($bts_number);
        $query = mysql_query($sql) or die(mysql_error());
...
if ($_GET['f'] == '40') { // редактор Докуметов
     $tx = "index.php?f=40&id=" . $_GET['id'];
	$id = $_GET['id'];
	//$sql = "SELECT * FROM documents WHERE bts_id=" . StrOrNull($id);
    //$query = mysql_query($sql) or die(mysql_error());
    //$row = mysql_fetch_array($query);
    if (isset($_GET['del'])) {
        DeleteInterfaceRow($last_index, $_GET['rn']);
...
if ($_GET['f'] == '41') { // редактор Докуметов
    $tx = "index.php?f=41&id=" . $_GET['id'];
	$id = $_GET['id'];
	$sql = "SELECT * FROM documents WHERE bts_id=" . StrOrNull($id);
    $query = mysql_query($sql) or die(mysql_error());
    $row = mysql_fetch_array($query);
...
if ($_GET['f'] == '42') { // редактор Докуметов
    $tx = "index.php?f=42&id=" . $_GET['id'];
	$id = $_GET['id'];
	$sql = "SELECT bts_number FROM bts WHERE id=" . StrOrNull($id);
    $query = mysql_query($sql) or die(mysql_error());
    $row = mysql_fetch_array($query);
...
if ($_GET['f'] == '43') { // Список Разрешений на БС
    $tx = "index.php?f=43";
}
if ($_GET['f'] == '44') { // Список Разрешений на Репитеры
    $tx = "index.php?f=44";
}
if ($_GET['f'] == '46') { // список Подвижных БС
    if (!empty($id)) {
        $_SESSION['pbs'] = $pbs_number;
        $tx = "index.php?f=47&id=$id";
    } else {
        $tx = $_SESSION['sections'][$last_index]['link'];
    }
}
if ($_GET['f'] == '47') { // Подробно данные ПБС
    $tx = "index.php?f=47";
	
}
if ($_GET['f'] == '48') { // Правка данных ПБС
    $tx = "index.php?f=48";
	
}
if ($_GET['f'] == '49') { // Внесение новой ПБС
    $tx = "index.php?f=49";
	
}
if ($_GET['f'] == '50') { // План по выбранной ПБС
    $tx = "index.php?f=50";
	
}
if ($_GET['f'] == '51') { // План по выбранной ПБС
    $tx = "index.php?f=51";
	
}
if ($_GET['f'] == '52') { // Редактировать мероприятие выбранной ПБС
    $tx = "index.php?f=52";
	
}
if ($_GET['f'] == '53') { // Редактировать мероприятие выбранной ПБС
    $tx = "index.php?f=53";
	
}
if ($_GET['f'] == '54') { // Поиск мероприятия по установке ПБС
    $tx = "index.php?f=54";
}
if ($_GET['f'] == '55') { // инфо бс
    if (isset($_POST['bts_number'])) {
        $bts_number = $_POST['bts_number'];
        $sql = "SELECT Id FROM bts WHERE bts_number=" . StrOrNull($bts_number);
        $query = mysql_query($sql) or die(mysql_error());
...
if ($_GET['f'] == '56') { // Блок справки по параметрам оборудования
    $tx = "index.php?f=56";
}
if ($_GET['f'] == '57') { // Блок учета дней работы ПБС LTE
    $tx = "index.php?f=57";
}
