...
$sql.= ",land_docs_minsk.bts as object_number"; //Номер объекта - БС, Репитер, ФТТХ,....
$sql.= ",changes"; //нарушена кодировка 
$sql.= ",table_name";
$sql.= " FROM history_land"; 
$sql.= " LEFT JOIN users"; //связка с таблицей пользователей
$sql.= " ON history_land.user_id=users.Id";
$sql.= " LEFT JOIN land_docs_minsk"; //связка с таблицей пользователей
$sql.= " ON history_land.object_id=land_docs_minsk.Id";
$sql.= " WHERE table_name like 'land_docs_minsk' AND object_id=$id ORDER BY history_land.Id DESC"; //склеивает строки в одну по символам ","
...
<head>
    ...
    <title>ИСТОРИЯ <?php echo $row['object_number']; ?></title>
                            ...
                            <div class="col-md-9">
                                <div class="row align-items-center ">
                                    <div class="col-md-3 arend">
                                        <a href="dogovor.php?Id=<?php echo $id; ?>"><button type="button" class="btn btn-danger" >НАЗАД</button></a>
                                    </div>
...
echo "<th>Id объекта</th>"; //table_name
...
for ($i=0; $i<mysql_num_rows($query); $i++) { //вывод данных из таблицы history
  ...
  echo "<td><b>".$row['object_id']."</b></td>"; //Id Объекта
