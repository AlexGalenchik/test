...
// формируем элементы
$searchinfo = array (  //Формируем поле для поиска по номеру объекта, зановсим в переменную
 'el_type' => 'text'
...
$adsearchinfo = array ( //Формируем поле для поиска по Региону, Району, Нас Пункту, Улице, номере дома...в строку. Заносим в переменную
 'el_type' => 'text'
,'value' => $searchstring 
,'id' => 'select_field'
,'name' => 'adsearch'
...
// основной запрос
$search_words=explode(' ',$searchstring); //Формирует массив из набора слов поискового запроса через пробелы 
if (strlen($search_words[0])>0)   { //Если первый элемент массива из поисковых слов имеет длину больше 0, то запускаем цикл прогона каждого поискового слова
  $search_where = " WHERE";
  for ($i=0; $i<count($search_words); $i++) { //цикл перебора от первого элемента массива из поисковых слов до конца его
    $w=$search_words[$i];   // Присвоение переменной значения каждого элемента массива из поисковых слов внутри цикла перебором
    if ($i>0) {$search_where.=" AND";} //Если в массиве из поисковых слов более одного слова то дополняем WHERE + bts_number OR area OR region....
  ...
  }  //тоесть каждое поисковое слово проверяется в нескольких полях
  $sql = "SELECT";
  $sql.= " bts.Id as Id";
  $sql.= ",bts_number";
  $sql.= ",site_type";
  ...
  $sql.= $search_where; //=WHERE (bts_number='$w' OR region LIKE '%$w%' OR area LIKE '%$w%' OR settlement LIKE '%$w%' OR street_name LIKE '%$w%' OR house_number LIKE '%$w%' OR place_owner LIKE '%$w%')";
  $sql.= $sort; //ORDER BY GET['sort'] - то выбранный пользователем фильтр
...
echo "<div id='inline'>";
echo "&nbsp;<i>Для поиска eNB на опорах A1 - внести <b>A</b>XXXXXX (6-значный номер БС)</i>";
echo "</div>";
echo "<br/>";
echo "<br/>";
echo "<div id='inline'>";
...
echo "&nbsp;&nbsp;&nbsp;<button type='submit'>выбрать</button><br/>";
echo "</form>";
echo "</div>";
