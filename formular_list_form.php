...
$sql = "SELECT id FROM users WHERE department='ОЧПиОС' AND (function='согласование')";  // список id согласующ ОЧПиОС
$query2 = mysql_query($sql) or die(mysql_error());
for ($i=0; $i<mysql_num_rows($query2); $i++ ) {
  $row = mysql_fetch_array($query2);
  $dep2_accept_ids[] = $row[0];
}     
// Добавлено для другого алгоритма согласования ОРТР, чтоб все согласующие согласовали (Борисов, Кострикин, Веренич)
$sql_dep3 = "SELECT id FROM users WHERE department='ОРТР' AND (function='согласование')";  // список id согласующ ОРТР (Борисов и Кострикин)
$query3 = mysql_query($sql_dep3) or die(mysql_error());
for ($k=0; $k<mysql_num_rows($query3); $k++ ) {
  $row3 = mysql_fetch_array($query3);
  $dep3_accept_ids[] = $row3[0]; //ноль так как нужны только номера id пользователей
...
if ($category == 'на подпись подразделения') {
  ...
  if ($_SESSION['department'] == "ОЧПиОС") {
    ...
    $where.= ") OR (repeater_id IS NOT NULL AND dep2_47.action='accept')";
    $no_fixed_cond = "AND action<>'fixed'";
  }
 
 if ($_SESSION['department'] == "ОРТР") $where = " WHERE dep3.action='accept'"; // Прошлый вариант кода (исходный)
	// Вставка нового кода, чтоб в отделе ОРТР все согласовали (Борисов, Веренич, Кострикин) перед подписью 
  //if ($_SESSION['department'] == "ОРТР") {
	  //$where = " WHERE (";
		//for ($k=0; $k<count($dep3_accept_ids); $k++ ) {
		//if ($k>0) {$where.= " AND "; }
	  //$where.= "dep3_$dep3_accept_ids[$k].action='accept' ";
...
if ($fm == 'r' || $fm == 'w')  {
  $info=array (
    'Выгрузить ФПД Репитеров' => "index.php?f=63"
  );
  ActionBlock($info);
}
...
if ($query) {
  ...
  for ($i=0; $i<mysql_num_rows($query); $i++) {
    ...
    if ($row['bts_id']>0) {
      ...
      echo "<td id='rs_td' style='width:300px'>".FormatAddress($row['type'],$row['settlement'],$row['street_type'],$row['street_name'],$row['house_type'],$row['house_number'],$row['area'],$row['region'])."$btsnum</td>";
     }
