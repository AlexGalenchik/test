...
//выбор данных из таблицы в виде массива
    ...
    function selectAll($conn)    {
		...
		If (!empty($_SESSION['dogovor_type'])) { //Фильтр по типу Договора (Добавлен фильтр по запросу Пилипенко и Лобкова)
...
$numeric_columns = ['P','Q','R','S','T','V','W', 'X','Y','Z','AA','AB','AC','AD','AE','AF','AG','AH'];// Колонки с числовыми значениями
...
//        заполняем массив значениями и убираем не нужные для вывода поля
        ...
        for($s=0; $s < count($abc); $s++) {
			...
			// задаем формат числа для выгрузки в эксель
			  ...
			  ($abc[$s] == "V" && isset($arrValue[$s]) && $arrValue[$s] != 0) OR 
			  ($abc[$s] == "W" && isset($arrValue[$s]) && $arrValue[$s] != 0) OR 
			  ...
			  ($abc[$s] == "AI" && isset($arrValue[$s]) && $arrValue[$s] != 0) 
...
//$page->getColumnDimension("AL")->setWidth(80);
$page->getColumnDimension("AM")->setWidth(80);
$page->getColumnDimension("AN")->setWidth(80);
//переносы
