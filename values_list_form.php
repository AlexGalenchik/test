<?php 
// определяем список для редактирования
if ($_GET['obj']=='regions') 
  {
  $table='regions';
  $field='region';
...
if ($_GET['obj']=='construction_5g_types') // 5G Добавлен
  ...
  $field='construction_type';
  $field_title='тип металлоконструкций 5G';
  }      
...
if ($bg == 'w' || $bm == 'w' || $fm == 'w') {
  $info=array (
    'Новая Запись' => "index.php?f=6&obj=$table&id=new"
  );
  ActionBlock($info);
}
...
echo "</table></div>";
echo "<p><a href='".$_SESSION['sections'][$_GET['ff']]['link']."'>Вернуться к выбору из списка</a></p>";
?>